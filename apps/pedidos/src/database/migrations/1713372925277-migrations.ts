...
export class Migrations1713372925277 implements MigrationInterface {
    name = 'Migrations1713372925277'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`orders_entity\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`name\` varchar(100) NOT NULL, \`price\` int NOT NULL, \`description\` varchar(255) NOT NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`status\` enum ('PENDING', 'PAYED', 'CANCELLED') NOT NULL, \`clientIdId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
